...
-- Now I move into the Customer Table --
...
ORDER BY 
  ...
  -- What I done above? First I commented out things I will not use or take into consideration this time.
  -- Second, I used AS to make everything shorter and easy to write.
  -- Then, I concatenated First and Last name to make 1 unique row.
  -- Used CASE clause to use Male or Female instead of M and F.
  -- Lastly I Joined in Customer City from Geography Table so I know the location of customers



-- Moving on with Product Table
...
-- First I commented out everything I will not use from now on.
-- Renamed some rows with AS Clause for so it's easier for me and everyone using the script later on
-- Included Product Category and Sub-Category, so the Manager can see results in both ways
-- Used ISNULL to change NULL values for Outdated in a new row. It's a choice so I don't handle that many Null values.
-- Then I did 2 Left Joins so I have available Product Subcategory Key and Product Category Key in the same table
-- This 2 Left Joins are from different tables, so we can access 2 different rows of information



-- Now we move into Internet Sales
...
-- Commented out things I will not use from now on
-- Add GetDate so we focused on Today and we move backwards 2 years as specified in the Manager documentation
-- Used the YEAR clause to so It doen't show as a full detailed date, and I only see Year 2021
-- LEFT now will read the 4 first digits in OrderDateKey and if the number is equal or smaller than -2 years, it will display it
-- I can use another function to do the same in a shorter way like: SELECT ... From ... WHERE date > DATEADD(year,-1,GETDATE())
